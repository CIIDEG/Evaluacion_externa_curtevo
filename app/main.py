...
@app.get("/admin/ai/pending-transcripts")
def api_pending_transcripts(request: Request, db: Session = Depends(get_db)):
    """Devuelve lista JSON de transcripciones pendientes o con error para procesado local."""
    me = auth.current_user(request)
    if not me or not auth.can(me, "ai_run"):
        raise HTTPException(403)
    rows = db.query(models.AudioTranscript).filter(
        models.AudioTranscript.status.in_(["pending", "not_configured", "not_implemented", "error"])
    ).all()
    return JSONResponse([{
        "id": t.id, "response_id": t.response_id, "form_code": t.form_code,
        "qid": t.qid, "filename": t.filename, "status": t.status, "error": t.error,
    } for t in rows])


@app.post("/admin/ai/update-transcript/{tid}")
def api_update_transcript(tid: int, request: Request, payload: TranscriptUpdate, db: Session = Depends(get_db)):
    """Recibe el resultado de una transcripción hecha en local y actualiza la BD."""
    me = auth.current_user(request)
    if not me or not auth.can(me, "ai_run"):
        raise HTTPException(403)
    t = db.query(models.AudioTranscript).filter_by(id=tid).first()
    if not t:
        raise HTTPException(404)
